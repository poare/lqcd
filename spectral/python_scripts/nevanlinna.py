...
def main():
    ...
    data_path = f'{root}/lqcd/spectral/hardy_optim_clean_submission/test/freqs_1.txt'
    ...
    print('Read in ' + str(Npts) + ' Matsubara modes at frequencies ~ ' + str([mp.nstr(x, 8) for x in Y]) + '.')
    for z in C:
    ...
    zspace = np.array([mp.mpc(z, eta) for z in zmesh])
    ...
    for k in range(Npts):
        ...
        err = abs(test_cont[k] - C[k])
        assert err < mp.mpf(1e-14), f'error is: {err}'

    out_path = f'{root}/Dropbox (MIT)/research/spectral/testing/output_1.txt'
