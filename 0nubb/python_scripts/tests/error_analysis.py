...
for q_idx, q in enumerate(q_list):
    ...
    props_k1_inv, props_k2_inv, props_q_inv = invert_props(props_k1_b, Nb = n_cfgs), invert_props(props_k2_b, Nb = n_cfgs), invert_props(props_q_b, Nb = n_cfgs)
    Zq[q_idx] = quark_renorm(props_q_inv, q_lat, Nb = n_cfgs)
    ...
    for mu in range(4):
        ...
        GammaV[mu], GammaA[mu] = amputate_threepoint(props_k2_inv, props_k1_inv, GV_boot[mu], Nb = n_cfgs), amputate_threepoint(props_k2_inv, props_k1_inv, GA_boot[mu], Nb = n_cfgs)
    ...
    for n in range(16):
        ...
        GammaO[n] = amputate_fourpoint(props_k2_inv, props_k1_inv, GO_boot[n], Nb = n_cfgs)
    SS = GammaO[0]
    PP = GammaO[15]
