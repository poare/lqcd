...
n_boot0 = 50
n_boot = n_cfgs
print('No bootstrapping, just running ' + str(n_boot) + ' samples.')
...
f['ZqbyZV_cfg'] = ZqbyZV
f['ZqbyZV'] = np.einsum('bq->qb', bootstrap(np.einsum('qb->bq', ZqbyZV), Nb = n_boot0))

f['Lambda_cfg'] = Lambda_list
f['Lambda'] = np.einsum('bijq->ijqb', bootstrap(np.einsum('ijqb->bijq', Lambda_list), Nb = n_boot0))
for i, j in itertools.product(range(5), repeat = 2):
    f['Z' + str(i + 1) + str(j + 1) + 'byZVsq_cfg'] = ZbyZVsq[i, j]
    f['Z' + str(i + 1) + str(j + 1) + 'byZVsq'] = np.einsum('bq->qb', bootstrap(np.einsum('qb->bq', ZbyZVsq[i, j]), Nb = n_boot0))
f['cfgnum'] = n_cfgs
f.close()
