...
# xlimits = [[0.0, 8.0], [0.0, 5.0]] if apsq else [[1.0, 4.1], [1.0, 4.4]]
...
def plot_fit_out(sp_idx, cvs, sigmas, fitter, fout, ylabel, ylimits, path):
    ...
    with sns.plotting_context('paper'):
        ...
        for cap in caps:
            cap.set_markeredgewidth(style['ecap_width'])
        plt.fill_between(x_band, fx_cvs + fx_sigmas, fx_cvs - fx_sigmas, color = fill_color, alpha = 0.2, linewidth = 0.0)
        plt.xlabel(xlabel, fontsize = style['fontsize'])
        plt.ylabel(ylabel, fontsize = style['fontsize'])
        plt.xlim(xlimits[sp_idx])
        plt.ylim(ylimits)
...
plot_fit_out(0, ZA_extrap_mu, ZA_extrap_sigma, ZA24I_fitter, ZA24I_fout, '$\mathcal{Z}_A$', ZA24I_range, \
                '/Users/theoares/Dropbox (MIT)/research/0nubb/paper/plots/rcs/fit_plots/ZA_24I.pdf')
...
plot_fit_out(1, ZA_extrap_mu, ZA_extrap_sigma, ZA32I_fitter, ZA32I_fout, '$\mathcal{Z}_A$', ZA32I_range, \
                '/Users/theoares/Dropbox (MIT)/research/0nubb/paper/plots/rcs/fit_plots/ZA_32I.pdf')
