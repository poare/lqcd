...
plot_raw = False            # True if we want to plot all raw data
# plot_raw = True
...
if plot_raw:
    for ii in range(len(ZqAV_data)):
        plot_rcs_raw(ZqAV_mu[ii], ZqAV_sigma[ii], ZqAV_labels[ii], ZqAV_ranges[ii],  \
                                '/Users/theoares/Dropbox (MIT)/research/0nubb/nnpp/plots/raw/ZqVA/' + ZqAV_stems[ii] + '.pdf')
    for n, m in itertools.product(range(5), repeat = 2):
        plot_rcs_raw(Z_mu[n, m], Z_sigma[n, m], '$\mathcal{Z}_{' + str(n + 1) + str(m + 1) + '}$', Z_range[n, m], \
                        '/Users/theoares/Dropbox (MIT)/research/0nubb/nnpp/plots/raw/Zops/' + Zlabels[n][m] + '.pdf')
        plot_rcs_raw(ZbyZVsq_mu[n, m], ZbyZVsq_sigma[n, m], '$\mathcal{Z}_{' + str(n + 1) + str(m + 1) + '} / \mathcal{Z}_V^2$', ZbyZVsq_range[n, m], \
                        '/Users/theoares/Dropbox (MIT)/research/0nubb/nnpp/plots/raw/ZopsbyZVsq/' + ZbyZVsqlabels[n][m] + '.pdf')

# Start fitting. Use fit forms here.
all_fit_forms = [
    Model(lambda params : lambda apsq : params[0] * apsq, 1, '(x)', 'c1'),
    Model(lambda params : lambda apsq : params[0] * (apsq ** 2), 1, '(x^2)', 'c2'),
    Model(lambda params : lambda apsq : params[0] * (apsq ** 3), 1, '(x^3)', 'c3'),
    Model(lambda params : lambda apsq : params[0] * (apsq ** 4), 1, '(x^4)', 'c4'),
    Model(lambda params : lambda apsq : params[0] / apsq, 1, '(1/x)', 'c5'),
    Model(lambda params : lambda apsq : params[0] * np.log(apsq), 1, 'log(x)', 'c6'),
    Model(lambda params : lambda apsq : params[0] * apsq * np.log(apsq), 1, '(x*log(x))', 'c7')
]
...
for ii, ZZ in enumerate(ZqAV_data):
    ...
    ZZ_params, ZZ_chi2, ZZ_dof, ZZ_fit_covar, ZZ_AIC, ZZ_model = process_fit_forms_AIC_best(x_axis, ZqAV_mu[ii], ZZ_cov_shrunk, all_fit_forms)
    plot_rcs_fitted(ZqAV_mu[ii], ZqAV_sigma[ii], ZZ_model, ZZ_params, ZZ_fit_covar, ZqAV_labels[ii], ZqAV_ranges[ii], \
                            '/Users/theoares/Dropbox (MIT)/research/0nubb/nnpp/plots/fits/ZqVA/' + ZqAV_stems[ii] + '.pdf')
...
for n, m in itertools.product(range(5), repeat = 2):
    ...
    ZZ_cov = get_covariance(np.einsum('kb->bk', ZZ))
    ZZ_cov_shrunk = shrinkage(ZZ_cov, lam)
    ZZ_params, ZZ_chi2, ZZ_dof, ZZ_fit_covar, ZZ_AIC, ZZ_model = process_fit_forms_AIC_best(x_axis, Z_mu[n, m], ZZ_cov_shrunk, all_fit_forms)
    plot_rcs_fitted(Z_mu[n, m], Z_sigma[n, m], ZZ_model, ZZ_params, ZZ_fit_covar, '$\mathcal{Z}_{' + str(n + 1) + str(m + 1) + '}$', Z_range[n, m], \
                    '/Users/theoares/Dropbox (MIT)/research/0nubb/nnpp/plots/fits/Zops/' + Zlabels[n][m] + '.pdf')
...
for n, m in itertools.product(range(5), repeat = 2):
    ...
    ZZ_cov = get_covariance(np.einsum('kb->bk', ZZ))
    ZZ_cov_shrunk = shrinkage(ZZ_cov, lam)
    ZZ_params, ZZ_chi2, ZZ_dof, ZZ_fit_covar, ZZ_AIC, ZZ_model = process_fit_forms_AIC_best(x_axis, ZbyZVsq_mu[n, m], ZZ_cov_shrunk, all_fit_forms)
    plot_rcs_fitted(ZbyZVsq_mu[n, m], ZbyZVsq_sigma[n, m], ZZ_model, ZZ_params, ZZ_fit_covar, '$\mathcal{Z}_{' + str(n + 1) + str(m + 1) + '} / \mathcal{Z}_V^2$', \
                    ZbyZVsq_range[n, m], '/Users/theoares/Dropbox (MIT)/research/0nubb/nnpp/plots/raw/ZopsbyZVsq/' + ZbyZVsqlabels[n][m] + '.pdf')
